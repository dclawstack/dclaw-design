...
### How do I update DClaw Design?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp design --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Design without Kubernetes?

Yes, for local development:

```bash
cd dclaw-design/frontend && npm run dev
cd dclaw-design/backend && uvicorn dclaw_design.main:app --reload
```
